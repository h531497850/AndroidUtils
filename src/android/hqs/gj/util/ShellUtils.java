...
public class ShellUtils {
	private static final String TAG = LogTool.makeTag(ShellUtils.class);
	...
	public static CmdResult execCmd(String[] cmds, boolean isRoot, boolean isNeedResultMsg) {
		...
		try {
			...
			Log.e(TAG, "Read proc info failed.", e);
		} catch (InterruptedException e) {
			Log.e(TAG, "Thread proc wait failed.", e);
		} finally {
			FileTool.close(dos);
			FileTool.close(errorResult);
			FileTool.close(successResult);
			if (proc != null) {
				proc.destroy();
			}
		}
