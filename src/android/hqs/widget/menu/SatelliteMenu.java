...
public class SatelliteMenu extends FrameLayout {
	...
	private List<SatelliteMenuBean> menuItems = new ArrayList<SatelliteMenuBean>();
	private Map<View, SatelliteMenuBean> viewToItemMap = new HashMap<View, SatelliteMenuBean>();
	...
	private void onClick() {
		if (plusAnimationActive.compareAndSet(false, true)) {
			if (!rotated) {
				...
				for (SatelliteMenuBean item : menuItems) {
	...
	private void resetItems() {
		if (menuItems.size() > 0) {
			List<SatelliteMenuBean> items = new ArrayList<SatelliteMenuBean>(
					menuItems);
			menuItems.clear();
			this.removeAllViews();
			addItems(items);
	...
	private static class SatelliteAnimationListener implements Animation.AnimationListener {
		...
		private Map<View, SatelliteMenuBean> viewToItemMap;

		public SatelliteAnimationListener(View view, boolean isIn, Map<View, SatelliteMenuBean> viewToItemMap) {
		...
		public void onAnimationStart(Animation animation) {
			if (viewRef != null) {
				...
				if (view != null) {
					SatelliteMenuBean menuItem = viewToItemMap.get(view);
					if (isInAnimation) {
	...
	private static class InternalSatelliteOnClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			SatelliteMenu menu = menuRef.get();
			if(menu != null){
				SatelliteMenuBean menuItem = menu.getViewToItemMap().get(v);
	...
	public Map<View, SatelliteMenuBean> getViewToItemMap() {
	...
	public void addItems(List<SatelliteMenuBean> items) {
		...
		int index = 0;
		for (SatelliteMenuBean menuItem : menuItems) {
